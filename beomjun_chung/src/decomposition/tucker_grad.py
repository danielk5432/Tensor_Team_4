"""
Tucker decomposition via gradient descent (Frobenius / L2 loss).
Uses optax Adam optimizer; initialization from HOSVD.
"""
...
def _params_from_tucker(result: TuckerResult) -> dict:
    params = {"core": result.core}
    for n, A in enumerate(result.factors):
        params[f"A{n}"] = A
    return params


def _tucker_from_params(params: dict, ndim: int) -> TuckerResult:
    factors = [params[f"A{n}"] for n in range(ndim)]
    return TuckerResult(core=params["core"], factors=factors)


def _make_step(optimizer, ndim: int):
    @jax.jit
...
def decompose(
    ...
    **kwargs,
    ...
    """Tucker decomposition via Adam + Frobenius loss. Returns (TuckerResult, errors)."""
    # Initialize with HOSVD
    params = _params_from_tucker(hosvd_decompose(tensor, rank))
    optimizer = optax.adam(learning_rate)
    opt_state = optimizer.init(params)
    step = _make_step(optimizer, tensor.ndim)
    ...
    for _ in range(max_iter):
        ...
        error = float(jnp.sqrt(loss)) / tensor_norm
        errors.append(error)
